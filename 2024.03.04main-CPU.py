    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        for data in data_loader_train:
            ...
            for i in range(len(intermediate_outputs)):
                pinjie = torch.cat([inputs_prefix[i], intermediate_outputs[i]])
                outputs = torch.cat([outputs, pinjie.unsqueeze(0)], dim=0)
                base_poses = torch.cat([base_poses, base_pose.unsqueeze(0)], dim=0)
            print('outputs', outputs)
            print('base_poses', base_poses)

            old_point_poses_of_the_endpoint = inputs[:, :6]
            print('old_point_poses_of_the_endpoint', old_point_poses_of_the_endpoint)
            next_point_poses_of_the_endpoint = IK.shaping(outputs)  # 对输出做shaping运算
            base_poses_of_the_robot = IK.shaping(base_poses)


            IK_loss_batch = torch.tensor(0.0, requires_grad=True)
            for i in range(len(inputs)):
                old_angle_solution, _, _ = IK.calculate_IK(next_point_poses_of_the_endpoint[i], base_poses_of_the_robot[i],
            ...
            #     calculated_IKloss, num_NOError1, num_NOError2 = loss.calculate_IK_loss(angle_solution,num_NOError1,num_NOError2)
            #     IK_loss_batch = IK_loss_batch + calculated_IKloss
