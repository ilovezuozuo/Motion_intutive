...
def distance_loss(global_output_list, output_t):
    ...
    if len(global_output_list) >=1:
        ...
        if 3 <= distance <= 5:
            ...
            distance_loss_at_t = distance_loss_at_t + abs(distance - torch.tensor(4.0)) * 5000
    else:
        distance = torch.sqrt(torch.sum((torch.tensor([8.0, 15.0], requires_grad=True) - output_t) ** 2))
...
def collision_free_loss(obstacles, output_t):
    ...
    if distance1 <= 8:
        collision_free_loss = collision_free_loss + abs(distance1 - torch.tensor(8.0)) * 8000
    else:
    ...
    if distance2 <= 8:
        collision_free_loss = collision_free_loss + abs(distance2 - torch.tensor(8.0)) * 8000
    else:
        collision_free_loss = collision_free_loss + torch.tensor(0.0)
    distance3 = torch.sqrt(torch.sum(torch.square(torch.tensor([40.0, 15.0]) - output_t)))
    if distance3 <= 8:
        collision_free_loss = collision_free_loss + abs(distance3 - torch.tensor(8.0)) * 8000
    else:
        collision_free_loss = collision_free_loss + torch.tensor(0.0)

    distance4 = torch.sqrt(torch.sum(torch.square(torch.tensor([15.0, 40.0]) - output_t)))
    if distance4 <= 8:
        collision_free_loss = collision_free_loss + abs(distance4 - torch.tensor(8.0)) * 8000
    else:
        collision_free_loss = collision_free_loss + torch.tensor(0.0)
...
def main():
    ...
    hidden_size = 32
    output_size = 2
    obstacles = [10., 10., 20., 20., 30., 30., 40., 40., 35., 10., 45., 20., 0., 20., 10., 30.]
    Obstacles = [[10., 10., 20., 20.], [30., 30., 40., 40.], [35., 10., 45., 20.], [0., 20., 10., 30.]]
    start_and_end_point = [8., 15., 50., 50.]
    inputs = torch.cat(
