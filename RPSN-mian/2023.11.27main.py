...
# 发现一个奇怪的问题，就是，感觉这个只是把RPSN拆开了，但是这个代码训练效果极差，缓慢上升到50%左右就开始徘徊震荡
# 参数跟RPSN1保持一致也是这样，不知道那里的问题，似乎numerror2不会下降。
# 目前知道的一个变化是，四个global统计量传入函数，希望计算各种情况的值
# 不知道是不是上面这个原因导致的，其他原因还要查一下，两边代码一样的按道理，可以一一对一下。
#
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        for data in data_loader_train:  # 读入数据开始训练
            ...
            if epoch == 400:
                ...
                utilities.checkpoints(model, epoch, optimizer, loss, '/home/yq/zgwang/ylpeng/programs/RPSN1true')
